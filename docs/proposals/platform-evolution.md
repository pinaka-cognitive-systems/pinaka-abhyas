...
Status: Proposed. Not a decision. No ADR has accepted or declined it.
Author: Navee Kandagatla, GitHub login naveen0905.
Submitted: 2026-09-16, as pull request #1.

Open conflict: Phase 1 of this proposal puts accounts and a server in scope.
`ROADMAP.md` lists cross-device sync and accounts as out of v1. ADR 0014 rests on no
personal data being processed. The brand trust planks say student data stays on the
device. That conflict is not resolved by this document.

Load-bearing choices here become ADRs before any code is written. This document is
the reasoning that would justify them.
