...
Status: Draft for review, version 3. Owner: product. Reviewer: engineering.
...
| Mock history | past mocks | Every mock the student has sat. | one row per mock, with type, date, and net score | Mock, Readiness |
...
| Comparison | other students | Where the student's readiness sits among students who opted in to telemetry. Labelled as a comparison, never a rank. | percentile band, sample size | Readiness, Aggregate |
...
| Export | backup | The student's whole history as one file, to keep or to move to another device. Importing merges by attempt id, so a file can be imported twice without harm. | format version, export time, app version, taxonomy version, attempts with their pack ids, reports, answers to in-app questions, settings; no name, no contact detail, no free text | Attempt, Report, Settings |
...
| Telemetry record | none | One anonymous attempt record, sent only when telemetry is on. | item content hash, taxonomy version, difficulty label, correct or not, coarse time bucket, item type, mode, device form factor, ability band with five levels | Item, Calibration, Aggregate |
...
| Verification tier | verified by | How an item was verified. Visible on every item a student sees. Machine-verified means a program re-derives the key. Expert-reviewed means two independent blind solves agree. An item with only structure checks stays in the contributor pool and never reaches a student. | tier, evidence | Item, Blind solve |
...
| Errata | what changed | The list of key corrections and quarantined items in a pack version, in plain words, shown to the student after an update. | pack version, corrected items, quarantined items, replacements | Pack, Item |
| Aggregate | none | A published file per exam with the spread of ability bands among students who opted in to telemetry, and its sample size. | exam, band counts, sample size, date | Comparison, Telemetry record |
...
| Mock | start a standard, hard, or pace mock; leave and return while the clock keeps running; submit; see the score, the marks breakdown, and the per-question review |
